...
graphtheme <- theme_minimal() + # Minimal theme
  theme(
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(),
    axis.line = element_line(colour = "black"), # Solid x and y axes
    text = element_text(size = 20) # Axis text size 20
  )
...
D <- rbinom(n, 1, 0.2) # treatment assignment (1 = treated, 0 = control)

# Covariates
family_income <- rnorm(n, mean = 50000, sd = 10000) - 20000 * D # Higher income for treated
parent_education <- rnorm(n, mean = 12, sd = 3) - 4 * D # Higher education level for treated

# Simulating outcome
# The true model is non-linear and includes interactions
health_outcome <- 50 + 5 * D + 0.01 * family_income + 0.5 * parent_education - 
...
p1 <- ggplot(data, aes(x = family_income, y = health_outcome, color = factor(D))) +
