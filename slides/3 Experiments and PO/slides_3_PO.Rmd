...
- the **average difference in potential outcomes** for those who were **not treated**
...
knitr::include_graphics(paste0(graphdir, "example_att.png"))
```
\vfill
\pause
**ATE**: Average of the last column \
**ATT**: Average of the green cells \
**ATU**: Average of the white cells
...
- what if we give the average person/firm a treatment
- This is interesting for medical trials and for some policy questions
...
\brf{ATU can be interesting for policy evaluation}
...
In most applications, we want to \brf{estimate the ATT}
\vfill
...
\vfill
Solution (?): **comparison of means**/simple difference in outcomes
\begin{equation*}
...
## Comparison by Treatment Status
Problem with SDO: units may \brf{select into treatment}

\begin{equation}\nonumber
\begin{array}{rcl}
E(Y_{i}|D_{i} = 1) &-& E(Y_{i}|D_{i} = 0)\\
 &=& E(Y_{i1}|D_{i} = 1) - E(Y_{i0}|D_{i} = 0)\\
 &=& \underbrace{E(Y_{i1}|D_{i} = 1) - \textcolor{brickred}{E(Y_{i0}|D_{i} = 1)}}_{ATT} + \underbrace{\textcolor{brickred}{E(Y_{i0}|D_{i} = 1)} - E(Y_{i0}|D_{i} = 0)}_{Selection bias}\\
\end{array}
\end{equation}
\vfill
