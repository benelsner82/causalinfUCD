...
For \brf{causal identification}, we require the assumption that the \brf{treatment} $D$ is as good as \brf{randomly assigned conditional on the covariates $X$} \vfill
...
\brf{Match each treated unit to a control unit} that has **exactly the same covariate values** \vfill This is called \brf{exact matching} and can be thought of as the \textbf{gold standard
for matching}
