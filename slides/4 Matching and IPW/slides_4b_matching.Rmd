...
## Estimation of the propensity score
Can use any \brf{standard probability model} to estimate the
propensity score, e.g. a logit model:
\begin{equation*}\label{eq23}
Pr\{D_{i} = 1|X_{i}\} = \frac{e^{\lambda h(X_{i})}}{1 + e^{\lambda h(X_{i})}},
\end{equation*}
where \(h(X_{i})\) is a \textbf{function of covariates with linear and higher order
terms.}
