...
$$ \text{Test score}_{ig} = \beta \; 1[\text{Small class}]_{ig} + \alpha_g + \varepsilon_{ig} $$
\vfill
$\beta$ is \brf{causally identified because of the random assignment} within schools
...
$$ Y_{it} = \beta X_{it} + \alpha_i + \varepsilon_{it} $$
\vfill
\brf{Suppose $X_{it}$ is a policy variable}: in some period $t$, a new policy is introduced in some  states 
...
\end{align*}
\vfill
With \textbf{constant state and time effects}, this \textbf{maps into the regression}
\begin{equation*}
...
Treated: \brf{non-Dutch/German/Belgian students} in Maastricht
Untreated: \brf{Dutch/German/Belgian students} in Maastricht
...
$$ Y_{it} = \alpha + \gamma \; \text{policy}_{t} + \lambda \; \text{treated}_{i} + \delta \; (\text{policy}_t \times \text{treated}_i) + \varepsilon_{it}$$
\vfill
